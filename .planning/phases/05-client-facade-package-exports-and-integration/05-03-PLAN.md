...
must_haves:
  ...
  key_links:
    ...
    - from: "test/integration/entry-points.test.ts"
      to: "dist/index.js"
      via: "dynamic import of built output"
      pattern: "import\\("
    - from: "package.json"
...
IMPORTANT: The entry point tests that import from dist/ require a build to exist. Add a note in the test file that `pnpm build` must run first. These tests should be in a separate describe block that can be skipped if dist/ doesn't exist.
  </action>
  <verify>
...
<task type="auto">
  <name>Task 2: Create consumer integration tests for provider-core, patient-core, and neuron patterns</name>
  <files>test/integration/entry-points.test.ts</files>
  <action>
Create test/integration/entry-points.test.ts as the consumer integration test suite. This verifies that the three authorized consumers can use Axon as intended.

The tests simulate real consumer import and usage patterns:
...
describe('Provider-core: taxonomy consumption', () => {
  it('can get actions for physician type for scope.permitted_actions', () => {
    // Simulates what provider-core will do during onboarding
    const actions = AxonTaxonomy.getActionsForType('physician')
    expect(actions.length).toBeGreaterThan(0)
    // Every action should have a valid dot-notation ID
  ...
  it('can validate individual action IDs from CANS scope', () => {
    // Provider-core will validate scope.permitted_actions strings
...
describe('Patient-core: provider discovery and connection', () => {
  it('can search for providers by specialty', () => {
    // Use the mock server to test discovery
    // Start mock, search for providers, verify results
  })

  it('can initiate a connection through the broker', () => {
    // Generate a key pair for the patient side
    // Create a signed connect request
    // Send to mock server's /v1/connect endpoint
...
describe('Neuron: registration and endpoint management', () => {
  it('can register a neuron', () => {
    ...
    // Verify registration_id and bearer_token returned
  })

  it('can register providers under a neuron', () => {
    // POST /v1/neurons/:id/providers
    // Verify provider_id returned
  })
...
For provider-core tests, import directly from source (taxonomy is a static class, no server needed).
...
IMPORTANT: These are integration tests from Axon's perspective -- they verify that the API surface Axon provides matches what consumers need. They do NOT import from the actual consumer packages (provider-core, patient-core, neuron). They simulate the consumer's import and usage patterns.

Import from source paths for direct API usage tests (AxonTaxonomy). Import createMockAxonServer from source path for HTTP-based tests. Use Node.js built-in `http` or `fetch` (available in Node 22) for HTTP requests to the mock server.
  </action>
  <verify>
